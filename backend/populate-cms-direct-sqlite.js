...
/**
 * Poblar CMS DIRECTAMENTE en SQLite - Sin API
 * Inserta TODO el contenido dinámico en la base de datos
 */
...
try {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
  console.log('║                                                            ║');
  console.log('║   🚀 POBLANDO CMS DIRECTAMENTE EN SQLITE                  ║');
  console.log('║                                                            ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');

  const db = new Database(dbPath);

  // Actualizar home_pages con todo el contenido
  const updateQuery = `
    UPDATE home_pages
    SET
      hero = ?,
  ...
  stmt.run(
    JSON.stringify(homePageData.hero),
    JSON.stringify(homePageData.impactHighlights),
    JSON.stringify(homePageData.identity),
    JSON.stringify(homePageData.missionVision),
    JSON.stringify(homePageData.activities),
    JSON.stringify(homePageData.programs),
    JSON.stringify(homePageData.attendedPersons),
    JSON.stringify(homePageData.eventCalendar),
    JSON.stringify(homePageData.catalog),
    JSON.stringify(homePageData.gallery),
    JSON.stringify(homePageData.supporters)
  );
  ...
  console.log('   • CMS: http://localhost:1337/admin');
