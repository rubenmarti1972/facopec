...
async function updateHomePageOnly() {
  const homeData = {
    data: {
      activities: [
  ...
  console.log('Intentando actualizar Home Page...');
  console.log('URL:', `${API_URL}/home-page`);
  console.log('Datos a enviar:', JSON.stringify(homeData).substring(0, 200) + '...');
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Home Page update failed: ${response.status} - ${responseText}`);
    }
...
async function main() {
  ...
  try {
    const result = await updateHomePageOnly();
