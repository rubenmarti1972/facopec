...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  private readonly fallbackCarouselSlides: HeroCarouselSlide[] = [
    {
      ...
      alt: "Bandera",
      caption: "Una misión con sentido social",
    },
    {
  ...
  carouselImages: CarouselImage[] = [
     {
      url: "assets/fotos-fundacion/bandera.jpg",
  ...
  eventCalendar: EventCalendarItemContent[] = [
    {
      ...
      description: "Programa de nivelación académica",
      eventDate: "2026-01-03T15:00:00",
    ...
    },
    {
      ...
      eventDate: "2026-01-05T17:00:00",
      location: "Sede FACOPEC",
      category: "reunion",
      color: "rose",
    },
    {
      title: "Fortalecimiento a líderes estudiantiles",
      description: "Formación para líderes estudiantiles en derechos humanos",
      eventDate: "2026-01-05T17:00:00",
      location: "Sede FACOPEC",
      category: "formacion",
      color: "purple",
    ...
    {
      title: "Aniversario Facopec",
      description: "Celebración del aniversario de FACOPEC",
  ...
  private applyHomeContent(content: HomePageContent): void {
    ...
    if (content.hero) {
      ...
      if (
        !this.fallbackService.isCmsDown() &&
        heroMediaUrl &&
        this.fallbackService.isValidImageUrl(heroMediaUrl)
      ) {
        // Solo usar imagen del CMS si es válida
        finalHeroImage = heroMediaUrl;
      } else if (
        !this.fallbackService.isCmsDown() &&
        (!heroMediaUrl || !this.fallbackService.isValidImageUrl(heroMediaUrl))
      ) {
        console.warn(
          "[HomeComponent] Hero image del CMS inválida, usando hardcodeada"
        );
      }
      ...
      if (mapped.length) {
        const fallbackEvents: EventCalendarItemContent[] = [
          ...
          },
          {
            title: "Fortalecimiento a líderes estudiantiles",
            description:
              "Formación para líderes estudiantiles en derechos humanos",
            eventDate: "2026-01-05T17:00:00",
            location: "Sede FACOPEC",
            category: "formacion" as const,
            color: "purple" as const,
  ...
  private applyHeroCarousel(hero: HeroSectionContent): void {
    ...
    hero.carouselItems?.forEach((item, index) => {
      ...
      if (imageUrl && this.fallbackService.isValidImageUrl(imageUrl)) {
        ...
        console.warn(
          `[HomeComponent] Imagen ${index} del carrusel inválida, usando fallback`
        );
    ...
    if (!hasValidSlides || slides.length === 0) {
      console.log(
        "[HomeComponent] No hay slides válidos del CMS, usando carrusel hardcodeado completo"
      );
      this.setHeroCarousel(this.fallbackCarouselSlides);
    } else {
  ...
  private setHeroCarousel(slides: HeroCarouselSlide[]): void {
    ...
    this.carouselImages = slides.map((slide, index) => ({
      ...
      title: slide.caption,
      fallbackUrl:
        this.fallbackCarouselSlides[index]?.image ||
